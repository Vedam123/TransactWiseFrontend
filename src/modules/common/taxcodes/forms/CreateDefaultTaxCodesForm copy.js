...
const CreateDefaultTaxCodesForm = ({ showModalWindow, headerId, onClose }) => {
  ...
  /*const handleCreateDefaultTaxCodes = async () => {
    try {
      ...
      console.log("Response datdfasdfdsa",response.data.results[0].message)
      ...
      if (response.data.message) {
        ...
        setSuccessMessage("");
        setErrorMessage("Some error");  // Clear any previous error message
      } 
    } catch (error) {
                ...
                <tr key={index} className="table-row">
                  <td>
                    <select
                      value={line.tax_id}
                      onChange={(e) => {
                        const selectedTax = taxCodesList.find(
                          (tax) => tax.tax_id === parseInt(e.target.value)
                        );
                        if (selectedTax) {
                          const updatedLines = [...lines];
                          updatedLines[index] = {
