...
export default function JournalResultsForm({ foundJournals }) {
  useEffect(() => {
    // Log a message when the component mounts
    logger.info(`[${new Date().toLocaleTimeString()}] ResultsContainer component has mounted.`);
    console.log("Search",foundJournals);

    // Log the number of explodedBOM items
    logger.debug(`[${new Date().toLocaleTimeString()}] Number of Items items: ${foundJournals.length}`);
